...
class PoseSaver:
    ...
    def save_current_pose(self, where: str):
        ...
        filename = os.path.join('/home/aidl/catkin_ws/src/IP200_ROS/ip200_navigation/config/', 'params' + str(self.current_file_location) + '.yaml')
...
class MainWindow(QMainWindow):
    def __init__(self, pose_saver):
        ...
        self.button0 = QPushButton('A 좌표에 저장')
        self.button0.clicked.connect(lambda: self.pose_saver.save_current_pose('0'))

        self.button1 = QPushButton('B 좌표에 저장')
        self.button1.clicked.connect(lambda: self.pose_saver.save_current_pose('1'))

        self.button2 = QPushButton('C 좌표에 저장')
        self.button2.clicked.connect(lambda: self.pose_saver.save_current_pose('2'))

        self.button3 = QPushButton('D 좌표에 저장')
        self.button3.clicked.connect(lambda: self.pose_saver.save_current_pose('3'))

        self.button4 = QPushButton('Docking 좌표에 저장')
        self.button4.clicked.connect(lambda: self.pose_saver.save_current_pose('docking'))
