...
class DockingController:
    def __init__(self):
        ...
        self.target_x = 0.02  # target x position (image center)
        ...
        self.docking_status = "Not docked"
    ...
    def is_docked(self):
        if self.docking_status == "Docked":
            rospy.loginfo(f"Docking Status: {self.docking_status}")
            return True
        elif self.docking_status == "Not Docked":
            rospy.loginfo(f"Docking Status: {self.docking_status}")
            return False
    ...
    def control_loop(self):
        ...
        small_angular_threshold = 0.02 # idle
        ...
        while not rospy.is_shutdown(): # rvec_z의 절대값이 3보다 작거나 같은 동안 반복
            ...
            # aruco 마커의 z축이 3 degree 이상 틀어지면 
            direction = self.rotate_clockwise_or_counter()

            if abs(self.rvec_z) > 6.0 and abs(self.rvec_x) < 150.0 :
                all_small_angular = all(abs(val[0]) < small_angular_threshold for val in control_stack)  # 모든 angular.z 값들이 기준값보다 작은지 체크
                ...
                rospy.loginfo(f'rvec_x: {self.rvec_x}')
                break
