...
class DockingController:
    ...
    def rotate_by_angle_difference(self, angle_difference):
        ...
        rospy.loginfo('================')
        rospy.loginfo(f'angle_difference: {angle_difference}')
        rospy.loginfo(f'current_angle: {self.current_angle}')
        rospy.loginfo(f'target_angle: {self.target_angle}')
        self.current_angle = self.current_angle * 180 / np.pi
        self.target_angle = self.target_angle * 180 / np.pi
        cmd = Twist()

        if angle_difference > 0:
            cmd.angular.z = 0.05  # 반시계방향 회전 -> z>0
        else:
            cmd.angular.z = -0.05  # 시계방향 회전 -> z<0
        ...
        while self.rotating and not rospy.is_shutdown():
            ...
            if abs(self.current_angle - self.target_angle) < self.angle_tolerance:
                ...
                self.cmd_pub.publish(cmd)
                rospy.loginfo(abs(self.current_angle - self.target_angle) * 180 / np.pi)
    ...
    def rotate(self, direction, target_angle_degree):
        ...
        initial_yaw = 100*self.current_angle
