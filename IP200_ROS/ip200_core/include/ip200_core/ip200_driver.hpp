...
#define PI 3.141592
#define WHEEL_DIAMETER 0.180
// #define WHEEL_WIDTH 0.4 // ip050
#define WHEEL_WIDTH 0.495 // model_solution
#define WHEEL_LENGTH 0.55 // model_solution
...
namespace aidl
{
	...
	private:
		...
		double motor_left_front_rpm_, motor_right_front_rpm_, motor_left_back_rpm_, motor_right_back_rpm_;
	...
	public:
											   ...
											   cur_x_(0), cur_y_(0), cur_th_(0), cur_vx_(0), cur_vy_(0), cur_vth_(0)
		{
			...
			motor_left_front_rpm_ = 0;
			motor_right_front_rpm_ = 0;
			motor_left_back_rpm_ = 0;
			motor_right_back_rpm_ = 0;
			...
			if (!ros::param::get("~wheel_length", wheel_length_))
				wheel_length_ = WHEEL_LENGTH;
			...
			cmd_rpm_publisher_ = nh_private_->advertise<ip200_msgs::RobotMotorMechanum>("cmd_rpm", 50);
		...
		void rpm_vel_callback(const ip200_msgs::RobotMotorMechanum &msg)
		{
			// velocity = (rpm / 60) * PI * diameter

			double rpm_left_front = msg.left_front;
			double rpm_right_front = msg.right_front;
			double rpm_left_back = msg.left_back;
			double rpm_right_back = msg.right_back;

			double cur_left_front = 0;
			double cur_right_front = 0;
			double cur_left_back = 0;
			double cur_right_back = 0;

			cur_left_front = (rpm_left_front / 60) * PI * wheel_diameter_ / GEAR_RATIO;
			cur_right_front = -1 * ((rpm_right_front / 60) * PI * wheel_diameter_ / GEAR_RATIO);
			cur_left_back = (rpm_left_back / 60) * PI * wheel_diameter_ / GEAR_RATIO;
			cur_right_back = -1 * ((rpm_right_back / 60) * PI * wheel_diameter_ / GEAR_RATIO);

			cur_vx_ = (cur_left_front + cur_right_front + cur_left_back + cur_right_back) / 4;
			cur_vy_ = (-cur_left_front + cur_right_front + cur_left_back - cur_right_back) / 4;
			cur_vth_ = (-cur_left_front + cur_right_front - cur_left_back + cur_right_back) / (4 * (wheel_width_/2 + wheel_length_/2));
		}
		...
		{
			// [m/s]
			ip200_msgs::RobotMotorMechanum motor_msg;

			double motor_left_front_vel = vx_ - vy_- (vth_ * (wheel_width_ / 2.0f + wheel_length_ / 2.0f));  // m/s
			double motor_right_front_vel = vx_ + vy_ + (vth_ * (wheel_width_ / 2.0f + wheel_length_ / 2.0f));
			double motor_left_back_vel = vx_ + vy_- (vth_ * (wheel_width_ / 2.0f + wheel_length_ / 2.0f));
			double motor_right_back_vel = vx_ - vy_ + (vth_ * (wheel_width_ / 2.0f + wheel_length_ / 2.0f));

			// [RPM]
			// rpm = (velocity / (PI * diameter)) * 60
			motor_left_front_rpm_ = motor_left_front_vel * 60.0f / (wheel_diameter_ * PI) * GEAR_RATIO;
			motor_right_front_rpm_ = -1 * (motor_right_front_vel * 60.0f / (wheel_diameter_ * PI) * GEAR_RATIO);
			motor_left_back_rpm_ = motor_left_back_vel * 60.0f / (wheel_diameter_ * PI) * GEAR_RATIO;
			motor_right_back_rpm_ = -1 * (motor_right_back_vel * 60.0f / (wheel_diameter_ * PI) * GEAR_RATIO);

			motor_msg.left_front = motor_left_front_rpm_;
			motor_msg.right_front = motor_right_front_rpm_;
			motor_msg.left_back = motor_left_back_rpm_;
			motor_msg.right_back = motor_right_back_rpm_;
		...
		void odom_compute(const ros::TimerEvent &e)
		{
			...
			delta_y_ = (1) * (cur_vx_ * sin(th_) + cur_vy_ * cos(th_)) * delta_t_;
			// delta_th_ = (0.9694) * cur_vth_ * delta_t_;								  // model_solution
			delta_th_ = (0.98) * cur_vth_ * delta_t_;                                 
